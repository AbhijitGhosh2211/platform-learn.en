...
Federated Audience Composition is a module for Adobe Real-Time Customer Data Platform (Real-Time CDP), Adobe Journey Optimizer. It is available with Adobe Real-Time CDP Composable Audiences ( A tailored solution for customers as a composable CDP). It enables data architects and data engineers to build and enrich audiences directly from [supported data warehouses](https://experienceleague.adobe.com/en/docs/federated-audience-composition/using/start/access-prerequisites){target="_blank"} without replicating data into Adobe Experience Platform. This tutorial provides hands-on guidance for technical users to connect enterprise data warehouses, create and enrich audiences, and activate them for personalized digital experiences.

## Business Context

SecurFinancial is a leading financial services firm. It leverages its wealth of customer data across disparate sources to personalize offers and campaigns for a large number of segments. They plan to use Adobe Real-Time CDP with Federated Audience Composition module which allows businesses to use their data warehouse while using Adobe Experience Platform to deliver personalized customer experiences. Key benefits include:
...
This guide shows you the steps for each of the activities taken to support various aspects of the business scenario. The goal is to provide you with activities that you can leverage in your environment, including:
...
To perform similar activities in your environment, ensure you have:
...
- **Sandbox Environments**: Create a sandbox in your instance.
